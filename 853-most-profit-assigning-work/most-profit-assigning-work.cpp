...
public:
    ...
    int maxProfitAssignment(vector<int>& difficulty, vector<int>& profit, vector<int>& worker) {
        ...
        // storing the profit with respect to their difficulty, difficulty as the index
        for(int i = 0; i < difficulty.size(); i++) mapping[difficulty[i]] = max(mapping[difficulty[i]], profit[i]);
        
        // As workers vec is sorted
        // The profit for a worker is obtained by using max of prev profit upto present profit
        int prev_profit = 0;
